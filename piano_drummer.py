...
for i in range(64):
  ...
  rand_seq = model.generate(torch.Tensor(out2), 
                                        target_seq_length=len(out2)+1,
                                        temperature=1,
                                        stop_token=640,
                                        verbose=False)
...
for i in range(63, 100):
  ...
  pcount = 0
  tdelta = next
  for j in [y % 128 for y in out1[-12:][0::2]]:
    time += j
